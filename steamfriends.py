...
class SteamFriends:
    def __init__(self):
        ...
        self.steamid_md = []    # 点击立即跳转到对应界面（Markdown格式）
        ...
        self.timezone = timezone(timedelta(hours=8))  # 设置为北京时间
    ...
    def get_friend_list(self):
        ...
        response = self.sess.get('https://api.steampowered.com/ISteamUser/GetFriendList/v1/', params=params)
        if response.status_code == 200:
            ...
            print('403Forbidden，请检查你的web_api和id的值')
            sys.exit(403)
        elif response.status_code == 500:
            print('服务器内部错误，请检查你的steamid的值')
            sys.exit(500)
    ...
    def get_friends_status(self):
        ...
        response = self.sess.get('https://api.steampowered.com/ISteamUser/GetPlayerSummaries/v2/', params=params)
        if response.status_code == 200:
            json_list = json.loads(response.text)
            users_list = json_list['response']['players']
            for user in users_list:
                self.steamid_num.append(user['steamid'])
                self.steamid_md.append('[' + user['steamid'] + '](https://steamcommunity.com/profiles/' + user['steamid'] + '/)')
    ...
    def create_from(self):
        ...
        for steamid in self.steamid_num:
            ...
            self.bfd.append(datetime.fromtimestamp(bfd_unix, tz=self.timezone).strftime('%Y-%m-%d %H:%M:%S'))
        data = {
            'Avatar': self.avatar,
            'Name': self.name,
            'steamid': self.steamid_md,
            'is_friend': is_friend,
            'BFD': self.bfd,
        ...
        original_content.append('\n')
        self.write_readme(df, original_content, len(original_content))
    ...
    def update(self):
        ...
        for num, sid in enumerate(self.steamid_md):
            if df[df['steamid'] == sid].empty:
                new_friend = {
                    'Avatar':  self.avatar[num],
                    'Name':    self.name[num],
                    'steamid': self.steamid_md[num],
                    'is_friend': '✅',
                    'BFD': datetime.fromtimestamp(self.friends_list[self.steamid_num[num]], tz=self.timezone).strftime('%Y-%m-%d %H:%M:%S'),
                    'removed_time': '',
                    'Remark': ''
                }
        ...
        for steamid in df['steamid']:
            if steamid not in friend_array:
                ...
                if df.loc[df['steamid'] == steamid, 'removed_time'].iloc[0] == '':
                    df.loc[df['steamid'] == steamid, 'removed_time'] = datetime.now(tz=self.timezone).strftime('%Y-%m-%d %H:%M:%S')

        df = df.fillna('')
        df = df.sort_values(by=['BFD', 'removed_time'], kind='mergesort')

        self.write_readme(df, self.content, self.table_start_index)
    ...
    @staticmethod
    def write_readme(df, content, table_start_index):
